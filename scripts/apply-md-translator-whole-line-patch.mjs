...
const injected = [
  ...
  "    // or rewritten. Inline token order may change when target-language grammar requires it,",
  "    // but leading HEADING/LIST/BLOCKQUOTE syntax is deterministically anchored at line start;",
  ...
  "      };",
  "      const leadingSyntaxTokenPattern = /^<<<(?:HEADING|LIST|BLOCKQUOTE)_\\d+>>>/;",
  "      const normalizeLeadingSyntaxToken = (sourceLine: string, translatedLine: string): string => {",
  "        const expected = sourceLine.match(leadingSyntaxTokenPattern)?.[0];",
  "        if (!expected || translatedLine.startsWith(expected)) return translatedLine;",
  "        const at = translatedLine.indexOf(expected);",
  "        if (at < 0) return translatedLine;",
  "        const without = translatedLine.slice(0, at) + translatedLine.slice(at + expected.length);",
  "        return expected + without.replace(/^\\s+/, \"\");",
  "      };",
  "      for (let i = 0; i < cleanedLines.length; i++) {",
  "        cleanedLines[i] = normalizeLeadingSyntaxToken(maskedContentLines[i], cleanedLines[i]);",
  "      }",
  ...
  "          if (!retryFailed) {",
  "            retryLines[0] = normalizeLeadingSyntaxToken(maskedContentLines[i], retryLines[0]);",
  "          }",
  "          if (retryFailed) {",
  "            if (tokenMismatch) {",
